...
const VideoCard = ({richThumbnail,publishedText,channelTitle,thumbnail,title,videoId,viewCount,type,fromwatchpage,channelId,description
  ...
  return (
         <>
         {
          type!=="channel"?
          (
            <Link className={` flex duration-1000 hover:scale-[1.01] ${fromwatchpage?" flex-row gap-2":"flex-col "}`} to={`/watchvideo/${videoId}`}>
            <div className=' relative'>
                <img src={type==="video"?thumbnail[0]?.url:thumbnail[3]?.url} className={`rounded-md ${fromwatchpage && "w-[200px] h-[120px]"} `}/>
            </div>
